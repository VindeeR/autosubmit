...
class Autosubmit:
    ...
    def _check_ownership(expid, raise_error=False):
        ...
        try:
            ...
            Log.warning(
                f"Current owner of experiment {expid} could not be retrieved. The owner is no longer in the system database.")
    ...
    def generate_as_config(
        ...
        template_files = [file.name for file in (read_files('autosubmitconfigparser.config') / 'files').iterdir() if
                          file.is_file()]
        ...
        try:
            ...
            with Lock(os.path.join(tmp_path, 'autosubmit.lock'), timeout=1) as fh:
                try:
                    ...
                    # Getting output type provided by the user in config, 'pdf' as default
                    try:
                        if not Autosubmit._copy_code(as_conf, expid,
                                                     as_conf.experiment_data.get("PROJECT", {}).get("PROJECT_TYPE",
                                                                                                    "none"), False):
                            return False
                    except AutosubmitCritical as e:
                        raise
