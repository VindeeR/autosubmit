...
class AutosubmitFigureDirective(code.CodeBlock):
    ...
    def run(self):
        ...
        local = os.environ['AUTOSUBMIT_CONFIGURATION']
        ...
        if self.options.get('name'):
            ...
            if Path(path_from).is_file():
                path_to = f"{local}/autosubmit/a000/conf/jobs_a000.yml"
                shutil.move(path_from, path_to)

            path_from = f"{self.env.srcdir}/{self.options.get('path')}/code/exp_{self.options.get('name')}.yml"
            if Path(path_from).is_file():
                path_to = f"{local}/autosubmit/a000/conf/expdef_a000.yml"
                shutil.move(path_from, path_to)
        ...
        for f in Path("{local}/autosubmit/a000/plot/").glob('*'):
            if f.is_file():
                path_from = f"{local}/autosubmit/a000/plot/{f.name}"
