...
def updateQueueStatus(queueStatus):
 output = commands.getoutput("ssh mn '%s ' " % 'mnq | grep -E \"eligible job|blocked job|active job\" | grep -v \"\-.*\"').split('\n')
 for line in output:
